...
// long perftOLD(int depth, board_t *pos) {
...
// void perft_divideOLD(int depth, board_t *pos) {
...
void perft(int depth, board_t *pos, long *leaf_nodes) {
    assert(check_board(pos));
    if (depth == 0) {
        *leaf_nodes += 1;
        return;
